...
set_light(lev) { return 1; }

reset(arg) { }

create() { seteuid(getuid(this_object())); reset(0); }
...
move(str) {
    int i = member_array(str, query_verb());
    string* destdir = explode(dest_dir[i-1], "#");
    string path = base_name(this_object());
    destdir[<1] = "/" + destdir[<1];
    dest_dir[i-1] = implode(destdir, "#");

    this_player()->move_player(dest_dir[i-1], dest_dir[i]);
}
...
query_object_class() { return OBJECT_CONTAINER; }
