 ...
 *  This version modified to communicate with external commands via sockets
 */
...
int eventCreateSocket(int extcmd, string args) {
    int x;

    Socket = new(class client);
    Socket->Blocking = 1;
    if( SocketType == -1 ) SocketType = STREAM;
    x = external_start(extcmd, args, (:eventReadCallback:),
		       (:eventWriteCallback:), (:eventAbortCallback:));
    if( x < 0 ) {
	eventSocketError("Error in external_start().", x);
	return x;
    }
    Socket->Descriptor = x;
}
