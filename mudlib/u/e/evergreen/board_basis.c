...
public string boardname, *threadprops;
...
string short() {
	if (!boardname) return "This board is nonfunctional!\n";
	return ::short(); }
...
sprintf("[%3d] %-50s <total: %d> (%s)", (i+1), threads[i], 
	sizeof(postings[threads[i]]), threadprops[i][0]); }
			ret += "\n\n";
		}
	}
...
int bnew(string str) {
	...
	if (sizeof(postings) >= MAX_POSTINGS) {
		...
		threadprops = threadprops[1 .. <0]; }
	postings += ([ str : ({ }) ]);
	write("Starting a new thread: "+str+"\n");
	return (int)this_object()->post(""+sizeof(postings)); }
...
void post_return(int thread, string *tbuf) {
	threadprops[(--thread)] += ({ capitalize(getuid(this_player())) });
	postings[(keys(postings))[thread]] += ({ tbuf });
...
int read(string str) {
	...
	write("Note written by: "+(threadprops[one][(two-1)]+"\n\n");
	write(postings[(keys(postings))[one]][two]+"\n");
	return 1;
}
