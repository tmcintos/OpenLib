...
void create()
{
  ...
  ({"destroy", "pummel", "kick the shit out of"}),     //hit_verbs
  ({"destroys", "pummels", "kicks the shit out of"}),  //hit_verbs2
  ({"wiff", "completely miss", "inflict a minor flesh wound"}),  //miss_verbs
  ({"wiffs", "completely misses", "inflicts a minor flesh wound"}), //miss_v_2
  ({"sword", "blade", "short sword"}),  //weapon_names
  WEAPON_SHORTSWORD,
  this_object(),     //this object.  =>
  0,                 //hands used (not set here)
  WEAP_SKILL_SHORTSWORD,
  }));
  set_hands(1);
 
  set_short("a sword");
  set_long(@ENDLONG
This is a stupid sword built to see if a person who hasn't coded in years
can still make a pointless stupid weapon.
ENDLONG);
 
  set_ids(({"sword","weapon"})); //please have "weapon" in
                                  //all weapons.
  set_weight(5);
  set_value(({ 50, 0 }));
}
