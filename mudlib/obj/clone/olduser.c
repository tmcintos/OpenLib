...
string query_home_dir() { return connection->query_home_dir(); }
...
{
  if(ob) ob->set_active();       // so it won't be destructed so easily
  connection = ob;
  return 1;
}
...
save_player()
{
  ...
  dir = SECURE_DATA_DIR "/user/body/" + username[0..0];
...
restore_player()
{
  ...
  file = SECURE_DATA_DIR "/user/body/"+ username[0..0] + "/" + username;
